...
const ReferenceGridOverlay: React.FC = () => {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-[9997]" style={{ opacity: referenceGrid.opacity }}>
      <div className="w-full h-full" style={{
          backgroundImage: `linear-gradient(to right, ${referenceGrid.color} 1px, transparent 1px), linear-gradient(to bottom, ${referenceGrid.color} 1px, transparent 1px)`,
...
const HelperGridOverlay: React.FC = () => {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-[9998] mix-blend-difference opacity-20">
      ...
      {symmetryX && <div className="absolute left-1/2 top-0 bottom-0 w-px bg-red-500 shadow-[0_0_8px_rgba(255,0,0,1)] mix-blend-normal opacity-100" />}
      {symmetryY && <div className="absolute top-1/2 left-0 right-0 h-px bg-red-500 shadow-[0_0_8px_rgba(255,0,0,1)] mix-blend-normal opacity-100" />}
    </div>
  );
};
...
const AnimationOverlay: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const renderOnionSkin = () => {
      ...
      for (let i = 1; i <= onionSkinFrames; i++) {
        ...
        ctx.drawImage(compositeCanvas, srcX, srcY, physicalW, physicalH, destX, destY, physicalW, physicalH);
      }
      ctx.globalAlpha = 1;
    };
    renderOnionSkin();
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-[9999]">
      {showOnion && <canvas ref={canvasRef} className="absolute inset-0 w-full h-full" style={{ imageRendering: 'pixelated' }} />}
      {(columns > 1 || rows > 1) && !isPlaying && (
        <>
          <div className="absolute inset-0 opacity-20 pointer-events-none" style={{
              backgroundImage: `linear-gradient(to right, #4488FF 1px, transparent 1px), linear-gradient(to bottom, #4488FF 1px, transparent 1px)`,
              backgroundSize: `${100 / columns}% ${100 / rows}%`
            }}
          />
          <div className="absolute border border-accent bg-[#4488FF]/5 shadow-[0_0_10px_rgba(68,136,255,0.5)] transition-all duration-100 pointer-events-none" style={{
              left: `${(currX / width) * 100}%`, top: `${(currY / height) * 100}%`, width: `${(frameW / width) * 100}%`, height: `${(frameH / height) * 100}%`
            }}
          />
          <div className="absolute inset-0 bg-bg-app/40 pointer-events-none" style={{
              clipPath: `polygon(0% 0%, 0% 100%, 100% 100%, 100% 0%, 0% 0%, ${(currX / width) * 100}% ${(currY / height) * 100}%, ${((currX + frameW) / width) * 100}% ${(currY / height) * 100}%, ${((currX + frameW) / width) * 100}% ${((currY + frameH) / height) * 100}%, ${(currX / width) * 100}% ${((currY + frameH) / height) * 100}%, ${(currX / width) * 100}% ${(currY / height) * 100}%)`
            }}
          />
        </>
      )}
...
export const LayerSurface: React.FC = () => {
  ...
  useEffect(() => {
    InputInterceptor.initShortcuts();
  }, []);
...
const LayerCanvas: React.FC<{ layer: any, zIndex: number, visible: boolean, opacity: number }> = ({ layer, zIndex, visible, opacity }) => {
  ...
  return (
    <motion.canvas
      ...
      style={{ zIndex: zIndex, mixBlendMode: layer.blendMode || 'normal', pointerEvents: 'none' }}
    />
  );
};